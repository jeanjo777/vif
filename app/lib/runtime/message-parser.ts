import type { ActionType, VifAction, VifActionData, FileAction, ShellAction, SupabaseAction } from '~/types/actions';
import type { VifArtifactData } from '~/types/artifact';
import { createScopedLogger } from '~/utils/logger';
import { unreachable } from '~/utils/unreachable';

const ARTIFACT_TAG_OPEN = '<vifArtifact';
const ARTIFACT_TAG_CLOSE = '</vifArtifact>';
const ARTIFACT_ACTION_TAG_OPEN = '<VifAction';
const ARTIFACT_ACTION_TAG_CLOSE = '</VifAction>';
const VIF_QUICK_ACTIONS_OPEN = '<vif-quick-actions>';
const VIF_QUICK_ACTIONS_CLOSE = '</vif-quick-actions>';

const logger = createScopedLogger('MessageParser');

export interface ArtifactCallbackData extends VifArtifactData {
  messageId: string;
  artifactId?: string;
}
...
interface MessageState {
  ...
  currentArtifact?: VifArtifactData;
  currentAction: VifActionData;
  actionId: number;
}
...
export class StreamingMessageParser {
  ...
  parse(messageId: string, input: string) {
    ...
    while (i < input.length) {
      if (input.startsWith(VIF_QUICK_ACTIONS_OPEN, i)) {
        const actionsBlockEnd = input.indexOf(VIF_QUICK_ACTIONS_CLOSE, i);

        if (actionsBlockEnd !== -1) {
          const actionsBlockContent = input.slice(i + VIF_QUICK_ACTIONS_OPEN.length, actionsBlockEnd);

          // Find all <vif-quick-action ...>label</vif-quick-action> inside
          const quickActionRegex = /<vif-quick-action([^>]*)>([\s\S]*?)<\/vif-quick-action>/g;
          let match;
          const buttons = [];
          ...
          i = actionsBlockEnd + VIF_QUICK_ACTIONS_CLOSE.length;
          continue;
        }
      }
        ...
        if (state.insideAction) {
          ...
          if (closeIndex !== -1) {
            ...
            this._options.callbacks?.onActionClose?.({
              ...
              action: currentAction as VifAction,
            });
          ...
          if (actionOpenIndex !== -1 && (artifactCloseIndex === -1 || actionOpenIndex < artifactCloseIndex)) {
            ...
            if (actionEndIndex !== -1) {
              ...
              this._options.callbacks?.onActionOpen?.({
                ...
                action: state.currentAction as VifAction,
              });
...
function createQuickActionElement(props: Record<string, string>, label: string) {
  const elementProps = [
    'class="__vifQuickAction__"',
    'data-vif-quick-action="true"',
...
function createQuickActionGroup(buttons: string[]) {
  return `<div class=\"__vifQuickAction__\" data-vif-quick-action=\"true\">${buttons.join('')}</div>`;
}
