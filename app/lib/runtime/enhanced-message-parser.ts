...
export class EnhancedStreamingMessageParser extends StreamingMessageParser {
  ...
  private _hasDetectedArtifacts(input: string): boolean {
    return input.includes('<vifArtifact') || input.includes('</vifArtifact>');
  }
  ...
  private _wrapInArtifact(artifactId: string, filePath: string, content: string): string {
    ...
    return `<vifArtifact id="${artifactId}" title="${title}" type="bundled">
<vifAction type="file" filePath="${filePath}">
${content}
</vifAction>
</vifArtifact>`;
  }
  ...
  private _wrapInShellAction(content: string, messageId: string): string {
    ...
    return `<vifArtifact id="${artifactId}" title="Shell Command" type="shell">
<vifAction type="shell">
${content.trim()}
</vifAction>
</vifArtifact>`;
  }
