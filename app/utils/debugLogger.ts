...
class DebugLogger {
  ...
  private async _collectAppInfo(): Promise<AppInfo> {
    ...
    try {
      if (typeof window !== 'undefined') {
        ...
        const workbenchStore = (window as any).__vif_workbench_store;
  ...
  private _getCurrentModel(): string {
    try {
      ...
      if (typeof window !== 'undefined') {
        const stored = localStorage.getItem('vif_current_model');
  ...
  private _getCurrentProvider(): string {
    try {
      if (typeof window !== 'undefined') {
        const stored = localStorage.getItem('vif_current_provider');
  ...
  private _getProjectType(): string {
    try {
      if (typeof window !== 'undefined') {
        const stored = localStorage.getItem('vif_project_type');
  ...
  private _getGitInfoFallback(): AppInfo['gitInfo'] {
    try {
      ...
      const stored = localStorage.getItem('vif_git_info');
...
export async function downloadDebugLog(filename?: string): Promise<void> {
  try {
    ...
    link.href = url;
    link.download = filename || `vif-debug-${new Date().toISOString().split('T')[0]}.txt`;
